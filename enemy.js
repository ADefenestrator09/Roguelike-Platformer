...
class Enemy{
  constructor(x,y,stats){
    ...
    this.info = `  enemies[${eNumber}] = new Enemy(${x},${y},enemyTypes.slime);`;
  }
  ...
  patrol(player,platforms){
    ...
    if(Math.abs(player.pos.x - this.pos.x) <= DETECTRANGE){
      if(!this.wasChasing){
        this.savedDirection = this.direction;
      }
      ...
      if(this.onGround){
        ...
        let checkX;
        if (this.direction > 0) { //if moving right, the front x position is equal to the x value plus width plus the leeway.
