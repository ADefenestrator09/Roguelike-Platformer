...
function createWeapon(id,name,damage,hWidth,hHeight,cooldown){
  return {id,name,type: ITEM_TYPES.WEAPON,stats:{damage},hWidth,hHeight,cooldown};
}
...
function drawUI(){
  ...
  drawSword(80,70,45,45,player.weaponDamage);
  strokeWeight(1);
...
//function for drawing the sword
...
//function for drawing the inventory slot
...
function mousePressed(){ 
  ...
  if(build){ //If in build mode
    newPlatform = new Platform(int(worldMouseX),int(worldMouseY),abs(pWidth),abs(pHeight),c,isSpike,isObjective);
    drawBuildPlat = true;
    hasPlaced = false;
  } 
  if(start){ //if in start menu
  ...
  if(win){ //if in win screen
    ...
    if(mouseX >= 300 && mouseX <= 450 && mouseY >= 225 && mouseY <= 275){
      levelSelector = true;
      win = false;
    }
  }
...
//   And it basically makes it so that the camera moves in steps of 100, using the Lerp function to move smoothly.
...
function changeColour(number){ //function for changing colour of selected levels
  if(levelSet == number){
    fill(25,25,25,200);
  } else{
    fill(50,50,50,100);
