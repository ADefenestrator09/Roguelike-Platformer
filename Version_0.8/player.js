...
class Player{
  ...
  //drawing the weapon hitbox
  drawWeaponHitbox(){
    let hWidth = this.equipped.weapon.hWidth;
    let hHeight = this.equipped.weapon.hHeight;
    fill(255,0,0,100);
    
    
    if(this.facingRight == true){
      rect(this.pos.x + this.pWidth,this.pos.y,hWidth,hHeight);
    } else{
      rect(this.pos.x-hWidth,this.pos.y,hWidth,hHeight);
    }
  }
  
  //damage in the hitbox
  weaponHitboxDamage(){
    let hWidth = this.equipped.weapon.hWidth;
    let hHeight = this.equipped.weapon.hHeight;
    let xPos = this.facingRight ? this.pos.x + this.pWidth : this.pos.x - hWidth;
    
    
    for(let enemy of enemies){
      

      if(enemy.pos.x < xPos + this.pWidth && 
          enemy.pos.x + enemy.w > xPos &&
          enemy.pos.y < this.pos.y + this.pHeight &&
          enemy.pos.y + enemy.h > this.pos.y && enemy.alive){
  ...
  update(gravity,jumpPressed,dashPressed,platforms){
    //Constants for the player's left and right.
    const playerLeft = this.pos.x;
    const playerRight = this.pos.x + this.pWidth;
    ...
    if(enemies){
      for(let enemy of enemies){
        ...
        if(enemy.pos.x < this.pos.x + this.pWidth && 
          ...
          if(Math.abs(this.vel.x) > VEL_THR){
            if (this.vel.x !== 0) {
              //constants for figuring out where to push
              const enemyLeft = enemy.pos.x;
              const enemyRight = enemy.pos.x + enemy.w;

              const overlapLeft = playerRight - enemyLeft;
              const overlapRight = enemyRight - playerLeft;

              if (overlapLeft > 0 && overlapRight > 0) {
                if (overlapLeft < overlapRight) {
                  this.pos.x -= overlapLeft + 1; //add a bit to the movement so that you don't get locked to the enemy
                } else {
                  this.pos.x += overlapRight + 1; //same thing but for the other side.
                }
                this.vel.x = 0;
              }
            }
    ...
    //if the coyote timer is greater than 1, reduce it.
