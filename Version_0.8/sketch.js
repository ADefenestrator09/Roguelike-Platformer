...
var edCooldown = 0; //enemy damage cooldown
var bCooldown = 0; //building cooldown
var rCooldown = 0; //resizing cooldown
var hitCooldown = 0; //hit coodown
...
function draw() {
  if(!start && !dead && !levelSelector && !win){ //In-level Screen
    ...
    //If you press P, you toggle between these 3 platform modes.
    if(keyIsDown(80) && rCooldown == 0 && buildMode == 0 && build){ //Normal Platform
      pHeight = 20;
      pWidth = 100;
      c = [0,220,70];
      rCooldown = COOLDOWNMAX;
      isSpike = false;
      isEnemy = false;
      buildMode = 1;
      isObjective = false;
    } if(keyIsDown(80) && rCooldown == 0 && buildMode == 1 && build){ //Spike
      pHeight = 15;
      pWidth = 15;
      c = [100,100,100];
      rCooldown = COOLDOWNMAX
      ...
      buildMode = 2;
      isObjective = false;
    } if(keyIsDown(80) && rCooldown == 0 && buildMode == 2 && build){ //Objective Block
      pHeight = 20;
      pWidth = 100;
      c = [255,100,100];
      rCooldown = COOLDOWNMAX;
    ...
    } if(keyIsDown(80) && rCooldown == 0 && buildMode == 3 && build){ //enemy 
      pHeight = 20;
      pWidth = 20;
      c = [0,255,0];
      rCooldown = COOLDOWNMAX;
    ...
    if(keyIsDown(32) && newPlatform && !hasPlaced){
      ...
      if(!isEnemy){
        ...
        console.log(`  enemies[${eNumber}] = new Enemy(${newPlatform.x},${newPlatform.y},enemyTypes.slime);`); 
    ...
    if(hitCooldown > 0){
      hitCooldown = decreaseCooldown(hitCooldown);
    }
    ...
    if(hitboxes){
      player.drawWeaponHitbox();
    }
    
    //attack
    if(!build && keyIsDown(32) && hitCooldown == 0){
      player.weaponHitboxDamage();
      player.drawWeaponHitbox();
      hitCooldown = player.equipped.weapon.cooldown;
    }
    
    //drawing enemies and making them function
    ...
    //drawing the new platform when in build mode along with the preview one.
    if(drawBuildPlat && build){
      newPlatform.oDraw();
    }else if(build){
      previewPlatform = new Platform(worldMouseX,worldMouseY,abs(pWidth),abs(pHeight),c,isSpike,isObjective);
  ...
  } else if(win){ //If you win, this is the win screen.
    fill(0,255,0);
    ...
    textSize(25);
    fill(0);
    text("Continue ->",135,257);
    text("Main Menu",312,257);
    
  } else if(levelSelector && !win && !dead){ //level selector screen
    background(135, 206, 235);
    stroke(0);
    fill(0);
    textSize(50);
